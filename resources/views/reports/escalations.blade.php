...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1>Daily Call Entry List - Escalations </h1>
            </div>
            <div class="col-sm-6">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-4 offset-4">
                                <form id="date-range" action="{{ route('admin.report.escalations') }}">
                                    @csrf
                        ...
                        <div class="table-responsive">
                            <table id="datatable-FaqCategory" class="table table-bordered table-striped table-hover datatable datatable-FaqCategory">
                                ...
                                <tbody>
                                    @foreach($escalations as $key => $entry)
                                    <tr data-entry-id="{{ $entry->id }}">
                                        ...
                                        <td>
                                            {{ $entry->escalation_department ?? '' }}
                                        </td>
                                        <td>
                                            {{ $entry->escalation_person_name ?? '' }}
                                        </td>
                                        <td>
                                            {{ $entry->escalation_outcome ?? '' }}
                                        </td>
                                        <td>
                                            {{ $entry->escalation_call_disposition ?? '' }}
                                        </td>
                                        <td>
                                            {{ $entry->escalation_comments ?? '' }}
                                        </td>
                                        <td>
                                            {{ $entry->user_id ?? '' }}
                                        </td>
